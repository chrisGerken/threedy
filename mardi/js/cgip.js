...
class Chaser {
	...
	constructor(color, target) {
		...
		this.tdo.position.set(this.random(-50,0),this.random(-50,0), -80);
		this.target = target;
	}
...
function colors(num, min) {
	  var rgb = rgbs(num, min);
	  var result = [];
	  for (var i = 0; i < num; i++) {
		  var color = (0x010000 * rgb[i].r) + (0x0100 * rgb[i].g) + rgb[i].b; 
		  result.push(color)
	  }
	  return result;
}

function rgbs(num, min) {
	  var unit = 0xff - min + 1;
	  var total = unit * unit * unit;
	  var result = [];
	  for (var i = 0; i < num; i++) {
		  var fract = (2 * i + 1) / (2 * num);
		  var lump = Math.floor(total * fract );
		  
		  var carry = Math.floor(lump / unit) * unit;
		  var b = lump - carry;
		  lump = Math.floor(carry / unit);

		  carry = Math.floor(lump / unit) * unit;
		  var g = lump - carry;
		  lump = Math.floor(carry / unit);
		  ...
		  var rgb = { "r": (r+min), "g": (g+min), "b": (b+min) };
		  result.push(rgb);
	  }
	  return result;
}
